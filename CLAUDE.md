...
- **Home order (recruiter skim path):** hero → `.cred-strip` → marquee → work grid → contact. The stats strip sits **directly under the hero** so the proof lands in the first screen at desktop; it used to sit after all five projects, where most visitors never scrolled. Keep the proof high.
...
- **Known-true facts (keep consistent):** GPA **3.87**; **10** robots designed; **16** podium finishes across 5 events; **4th** national Plastic Antweight (2024 RCL). Machining is **outsourced** (not in-house). Dog is **Mocha**. No "hammer" weapons.
  - The **16** is the *club's* count ("The team placed on the podium 16 times", in the Experience entry and on the résumé), so label it **"Team podium finishes"** wherever it appears alone, as the home strip and the brief do. See §12: the site's own awards table doesn't add up to it yet.
  - **Graduating May 2029**; started Sep 2025; **seeking a Summer 2027 co-op** (hardware / robotics). Confirmed by the owner 2026-09. These must stay in sync everywhere they appear. The co-op is in the hero telemetry "Seeking" cell, the contact lead, the print brief's `.ps-lead`, `<meta name="description">` and `og:description`. The grad date is in the About "Graduating" cell, the Education card date and the brief's `.ps-when`. The **class year ('29)** is in the hero eyebrow, the brief's `.ps-role` and, as pixels, in `images/web/og-card.jpg`.
...
- **Podium counts don't reconcile (raised 2026-09, open).** The Experience entry and the résumé say the team took 16 podiums, "4 firsts, 7 seconds, 5 thirds", across 5 events. The site's own awards table lists 4 firsts, 8 seconds and no thirds, across about 10 differently named events. The résumé's combat entry says "8 regional podium finishes". A recruiter who reads both will notice. Don't guess which is right; ask.
